...
# ------------------------Функции связанные с формой-----------------------------
        def on_btnSave_clicked():
            ...
            e_reg = re.compile(r"\s*e\n\s*\{\n\s*\S*\n\s*\S*\s*\S*\n\s*\S*\s*\S*\n\s*\}")
            e_mas = e_reg.findall(data)
            e1="$U;"+"\n"+"        "
            e2="tolerance"+"       "+toler_txt+";"+"\n"+"        "
            e3="relTol"+"          "+rT_txt+";"+"\n"+"    "
            e_txt_add = "\n"+"\n"+"    "+"e"+"\n"+"    "+"{"+"\n"+"        "+e1+e2+e3+"}"
            ...
            parent.item = QtGui.QListWidgetItem("Сохранен файл: fvSolution", parent.listWidget)
...
# -------------------------------Разметка формы----------------------------------
        ...
        solver_frame.setFixedSize(180, 40)
